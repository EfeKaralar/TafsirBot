"""
quran.py — Quran text lookup from the quran-json dist files.
...
class QuranRef:
    ...
    def __init__(self, dist_dir: str | Path | None = None) -> None:
        # Resolved from Settings rather than read from os.environ here — library
        # modules do not touch the environment directly.
        if dist_dir is None:
            from tafsirbot.settings import get_settings

            dist = get_settings().quran_json_dist
        else:
            dist = Path(dist_dir)
